...
#'
#' This function opens \code{preview.html} in a web browser. This file displays map tiles in a Leaflet widget.
#' The file is created when \code{tile} is called to generate the map tiles, unless \code{viewer = FALSE}.
#' Alternatively, it is created (or re-created) subsequent to tile creation using \code{tile_viewer}.
#'
...
view_tiles <- function(tiles){
  ...
  if(!file.exists(file)){
    stop(paste("Cannot find preview.html.", "Tiles may have been generated with `viewer = FALSE`.",
...
#' @param width \code{NULL} (default) for geospatial map tiles. The original image width in pixels for non-geographic, simple CRS tiles.
#' @param height \code{NULL} (default) for geospatial map tiles. The original image height in pixels for non-geographic, simple CRS tiles.
...
#' @param ... additional optional arguments include \code{format = "tms"} if necessary, and \code{lng} and \code{lat} for setting the view longitude and latitude. These three arguments only apply to geographic tiles. Viewer centering is \code{0, 0} by default.
#'
...
tile_viewer <- function(tiles, zoom, width = NULL, height = NULL, georef = TRUE, ...){
  index_js <- .index_js(tiles, zoom, width, height, georef, ...)
  if(inherits(index_js, "character")) index_js <- paste(.raster_coords, index_js, sep = "\n\n")
...
.index_js <- function(tiles, zoom, width, height, georef, ...){
  ...
  if(is.null(width) | is.null(height)){
    ...
    x <- paste0("function init () {
      ...
      L.tileLayer('", basename(tiles), "/{z}/{x}/{y}.png', { minZoom: ", zoom[1], ", maxZoom: ",
      zoom[2], ifelse(fmt == "tms", ", tms: true", ""), " }).addTo(mymap)
    }")
  } else {
    ...
    x <- paste0("function init () {
      ...
      "L.tileLayer('", basename(tiles), "/{z}/{x}/{y}.png', { noWrap: true }).addTo(map)
    }")
  }
  x
}
