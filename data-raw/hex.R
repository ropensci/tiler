...
hex_plot <- function(out, mult = 1){
  g <- sticker(img, 1, 1.125, 0.78, 0.78, pkg, p_x = 1.0, p_y = 0.67, p_color = "black", p_size = mult * 47, h_size = mult * 1.2, h_fill = "#FF6C00",
          h_color = "black", url = "", filename = out) +
    geom_url(url = paste0("ropensci.github.io/", pkg), x = 1.01, y = 1.125, color = "white", size = mult * 3, angle = -23.5)
