...
Python is required as well as the `gdal` library for Python. For Windows users, this is not the same as simply having `rgdal` installed through R. You need to install `gdal` so that it is accessible by Python. One of the easiest and typically recommended ways to do this in Windows is to install [OSGeo4W](https://trac.osgeo.org/osgeo4w/) (include QGIS option)- or the other way around: install [QGIS](https://qgis.org/en/site/forusers/download.html) and include the OSGeo4W option. Taking one of these approaches brings with it all the required Python and `gdal` library functionality. See `?tiler_options` for details.

On Windows for potential user convenience, the `tiler` package will perform a check on package load for the `OSgeo4W.bat` utility in a couple locations (see `?tiler_options`). If found, its path is set in `tiler_options`. If it is not found at one of these locations, the path remains unspecified and `tiler` will fall back on only calling the system Python installation. The fall back is less likely to work unless you are very diligent and know what you are doing and have set up all required resources for this Python installation. Bottom line: just use OSGeo4W for ease of use unless you are already set up in another manner.
...
Notice in the example above, there is no log output regarding the preparation of an intermediate raster. In the earlier examples, an intermediate raster had to be created and saved to disk temporarily and this temporary file was used for the tiling process. This is required in any case where `file` must be altered in some way. In this example, there was no `crs` input override, no `crs_out` reprojection required, and no conversion of a single-layer raster input file to a three or four band RGB/A raster with tile color information.

This `file` required no pre-processing by `tile` and was immediately passed on to `gdal2tiles` without any need to create an intermediary file. This is something to keep in mind if you have an extremely large input file. If you prepare it as an RGB raster in the desired projection and do not need to alter it in any way, you can avoid the overhead of manipulating it in memory in R and writing out an intermediary version prior to tiling.
...
Geographic provider tiles based on the low resolution example map included in `tiler` of the 48 contiguous US states.

```{r map1}
library(leaflet)
tiles <- "https://leonawicz.github.io/tiles/us48lr/tiles/{z}/{x}/{y}.png"
leaflet(
  options = leafletOptions(minZoom = 3, maxZoom = 7), 
...
Non-geographic provider tiles based on a Star Trek fictional galaxy map.

```{r map2}
tiles <- "https://leonawicz.github.io/tiles/st2/tiles/{z}/{x}/{y}.png"
leaflet(
  options = leafletOptions(
    crs = leafletCRS("L.CRS.Simple"), minZoom = 0, maxZoom = 7, attributionControl = FALSE), 
    width = "100%", height = "400px") %>% 
